...
- Amazon imports must persist otherwise-valid order items into the app even when the corresponding ShipStation Notes to Buyer update cannot be completed. A failed note update is a non-blocking ShipStation synchronization warning, not an import failure.
...
- When a ShipStation Notes to Buyer update cannot complete, the Amazon completion dialog must remain a successful import result when no blocking failures occurred, show the warning count and safe per-shipment warning details, and identify the affected Amazon order number and `Notes to Buyer` action without exposing customer data or upstream error text.
