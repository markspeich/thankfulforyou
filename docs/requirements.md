...
- Every discovered Etsy transaction import attempt must also append one server-only, append-only audit row correlated by an import-run UUID. The row must retain the complete receipt, transaction, listing, image, normalized-item, persistence-decision, and failure payloads available at the attempt stage; it must never be exposed through browser APIs or ordinary logs. Failure to write this audit row must be logged only with safe run/receipt/transaction identifiers and must not change the import outcome. Trusted operators need a read-only exact-order admin query for these rows in local and production environments.
...
- When an order item's imported product/listing title contains `Keychain` (case-insensitive substring matching), its SVG export notes column must include only the exact word `Keychain` for this rule, on its own line alongside any other applicable notes. The product title itself must not be added to the notes; the export payload should carry only a boolean indicator for this rule. Apply the rule to single-design and batch export/copy output and every quantity copy of the matching item, and leave other items in the same order unaffected unless their own titles match. Missing titles must not add the note.
...
- Etsy and Amazon imports must persist the marketplace order-placement timestamp on each order item. Etsy uses the receipt creation timestamp; Amazon uses ShipStation's marketplace payment date. Existing and manually created order items may have no order date.
- Grouped order summaries and details must expose the order date, using the earliest valid item order date when a grouped order contains differing values.
...
- When an imported design initially has one text line and the operator splits it into additional lines, each new line without its own imported font selection or a preset font explicitly targeting that line must inherit line 1's effective font. Matching preset line rules and listing-specific line overrides count as explicit targets; general preset line defaults do not. An explicit but unrecognized imported font selection must continue to leave that line's preset font unchanged rather than inheriting line 1.
...
- Future fixed-design ingestion should preserve each uploaded SVG unchanged while generating a versioned normalized derivative for preview and production use. Normalization should sanitize the SVG, make units and transforms explicit, convert supported shapes to clean compound paths, and preserve holes through an appropriate fill rule; existing uploads should remain compatible through read-time normalization until regenerated derivatives are available.
...
- September 15, 2026: Individual normalization or persistence failures must preserve the sync timestamp so the next on-demand import revisits unresolved orders. Optional listing/image enrichment may still fall back to receipt data when a complete order can be persisted. Abort, authorization, global, lease, discovery, transport, and size-limit failures must not advance the timestamp.
- Temporary Etsy order persistence failures must receive up to two automatic retries with one- and two-second delays. Validation/authorization failures must not be retried automatically, and cancellation or loss of the import lease must stop the run.
- Retrying an order must safely recover a missing design or untouched draft with no saved lines. Design initialization and line creation must be transactional, preserve operator edits and protected designs, and not create duplicate order items.
- Failed Etsy order numbers must be visible in the import result, with guidance to run Import again to retry unresolved orders. Import remains on demand; retries during a run do not introduce background scheduling.
...
- ShipStation automation should add the `Customization Needed` tag when an Amazon order is imported into ShipStation.
- The operator-initiated ShipStation-backed Amazon import should process only pending shipments in the configured Amazon ShipStation store that carry the exact `Customization Needed` tag.
...
- The Amazon importer must not add or depend on the legacy `Amazon Customization Imported` tag.
- A ShipStation shipment should have its `Customization Needed` tag removed only after its Notes to Buyer update and all app item persistence operations have succeeded or been recognized as already complete, and every shipment line item has usable customization text.
- If any shipment line item lacks usable customization text, or persistence or Notes to Buyer synchronization fails, the `Customization Needed` tag must remain so the shipment stays visible and retryable.
- One Amazon shipment's failure must not stop other eligible shipments; failed shipments should retain the `Customization Needed` tag and remain retryable.
- Amazon import completion feedback should separately report processed shipments, imported app items, existing app items, shipments not awaiting customization, customization-needed items, non-blocking ShipStation synchronization warnings, and failures.
- When a ShipStation Notes to Buyer update or `Customization Needed` tag removal cannot complete, the Amazon completion dialog must remain a successful import result when no blocking failures occurred, show the warning count and every supplied safe per-shipment warning detail, and identify each affected Amazon order number and synchronization action without exposing customer data or upstream error text. Service, API, browser validation, and UI rendering must not impose a fixed count cap on safe warning details. Safe warning details must remain visible when the same completion also reports one or more import failures.
...
- When a fixed SVG item's `Backing Border` setting is enabled, its backing layer should remain red while all visible fixed artwork, including fills and outline strokes, renders white in the preview to match the raised text layer. When the setting is disabled, the preview should preserve the fixed artwork's authored colors.
...
- Etsy imports must treat the variation label `Heart Color` as an alias of `Color` when populating the imported order item's color.
- Amazon and Etsy imports must extract the customer's badge-reel type selection from marketplace fields labeled `Badge Reel` or `Badge Reel Type` and normalize equivalent marketplace answers to one stable, marketplace-neutral identifier.
- Canonical badge-reel type identifiers must be stored in a dedicated nullable `order_items.badge_reel_type_id` field. Supported canonical types are `swivel-alligator`, `heavy-duty-belt-clip`, `mri-safe`, `belt-clip`, and `heavy-duty-carabiner`. Marketplace spellings that mean the same thing must resolve to the same ID; for example, `Swivel Alligator Clip` resolves to `swivel-alligator`, while `Belt Clip-Heavy Duty` and `Heavy Duty Belt Clip` resolve to `heavy-duty-belt-clip`.
- Manual Amazon and Etsy clipboard exports must retain the raw option value when a badge-reel choice is present but unrecognized, so a newly introduced marketplace value can be diagnosed without recapturing private order-page data.
- Badge-reel type matching must ignore case, surrounding whitespace, and punctuation differences. Missing or unrecognized selections must not fail an import; the canonical field remains null and the original marketplace value remains available in existing source metadata.
- A re-import may populate a missing canonical badge-reel type but must not replace an existing non-null canonical type or overwrite saved design data.
- Historical Amazon and Etsy order items should be backfilled from their retained marketplace source metadata where a known badge-reel type can be resolved.
- The first public badge-reel candidate is authoritative even if blank or unrecognized. Future Amazon imports retain only its presence and canonical outcome in a compact marker, without duplicating raw values or internal fields; accepted response arrays and design text remain unchanged. Historical Amazon rows without the marker use retained accepted responses.
- Historical untagged Etsy rows are identified by their retained variations array. Marketplace-tagged rows use only their own marketplace's source shape; malformed retained arrays are treated as empty. Backfill must preserve existing non-null IDs, source JSON, order status, designs, and batch membership.
- Concurrent duplicate imports must preserve the already stored order and design atomically, and import counts must include only newly inserted rows. Compact Orders metadata must carry a safe candidate-presence signal so unresolved selections display `Unrecognized` before detail loading and after detail failures, without exposing raw answers.
...
- If a production-batch save was committed but its response was lost, and the authoritative remote design exactly matches the attempted design apart from revision and audit metadata, the app should treat the remote design as confirmation that the save succeeded.
- If production-batch save recovery fails, the app should stop further saves for that page session, keep the current draft visible, and show an in-app dialog recommending an app reload with `Reload App` and `Keep App Open` actions.
- Production-batch save failures and recovery attempts should write useful browser-console diagnostics including the failure phase, attempt, retryability, HTTP status when available, error name and message, batch id, and affected order-item ids. These diagnostics must not include access tokens, request headers, full snapshots, design text or settings, imported source metadata, or other customer data.
- In the Orders workspace selected-order item cards, each card title should be `Order Item`, with imported listing text in the header, imported Etsy listing image in the left body column, and saved design preview in the right body column. The bottom metadata should show `Personalization: <design text>`, imported color, badge-reel type, and quantity as stacked rows in that order. Recognized types use their canonical display label; missing types display `Not set`; retained but unrecognized marketplace values display `Unrecognized`.
