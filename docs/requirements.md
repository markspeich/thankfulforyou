...
- Canonical badge-reel type identifiers must be stored in a dedicated nullable `order_items.badge_reel_type_id` field. The initial canonical type is `swivel-alligator`, displayed as `Swivel Alligator`, with marketplace aliases including `Swivel Alligator` and `Swivel Alligator Clip`.
