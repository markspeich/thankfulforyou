...
- When a ShipStation Notes to Buyer or processed-tag update cannot complete, the Amazon completion dialog must remain a successful import result when no blocking failures occurred, show the warning count and every supplied safe per-shipment warning detail, and identify each affected Amazon order number and synchronization action without exposing customer data or upstream error text. Safe warning details must remain visible when the same completion also reports one or more import failures.
