...
- When a change adds or modifies any database-dependent behavior, create a checked-in Supabase migration file as part of the same work before considering the feature complete. This includes new or changed tables, columns, constraints, indexes, triggers, functions, RLS policies, grants, storage buckets, storage policies, seed data required by deployed code, or API/client code that reads or writes a schema shape not already present in production.
- Create migration files with the Supabase migration command from the repository root, for example `npx supabase migration new <descriptive_name>`, then edit the generated SQL file. If the migration command is unavailable or fails, stop and ask the user how to proceed instead of inventing a migration filename by hand.
- After creating a migration file, verify it locally with the local Supabase workflow where practical, such as `npm run prepare:local` or the relevant local database test command, so a fresh database receives the schema the code expects.
- Before deploying work that includes a schema migration outside the `finish this worktree` workflow, explicitly tell the user the migration file path and prompt them to have the agent apply/run that file against the target database. For production, name the live project ref `oezjskcygvfyezvoulzw`, state whether the migration is additive or destructive, and ask before applying it.
- `finish this worktree` authorizes applying required additive checked-in migrations to the production project `oezjskcygvfyezvoulzw` as part of the finish workflow; report each migration path and apply it before pushing dependent code. Destructive migrations always require a separate explicit confirmation, even during `finish this worktree`.
- Do not rely on deployed code, Vercel environment variables, or a local reset as evidence that production has the matching schema. After applying a migration, verify production migration history or the production schema before continuing the finish workflow.
