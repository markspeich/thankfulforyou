...
- Before deploying or finishing any work that includes a schema migration, explicitly tell the user the migration file path and prompt them to have the agent apply/run that file against the target database. For production, name the live project ref `oezjskcygvfyezvoulzw`, state whether the migration is additive or destructive, and ask before applying it.
