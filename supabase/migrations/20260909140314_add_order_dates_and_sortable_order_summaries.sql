...
create index if not exists order_items_workspace_order_date_idx
  on public.order_items (workspace_id, order_date, id);

drop function if exists public.list_workspace_order_summaries(uuid, uuid, text, text, text, integer, text, text);
...
as $$
  ...
  select
    hydrated.group_id, hydrated.sort_key,
  ...
  select coalesce(jsonb_agg(
    case when jsonb_typeof(item.value) = 'object' and jsonb_typeof(item.value -> 'orderItem') = 'object'
      then jsonb_set(item.value, '{orderItem}', (item.value -> 'orderItem')
        - 'amazon_customization_json' - 'etsy_import_diagnostics' - 'order_date')
