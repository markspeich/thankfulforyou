...
test("disconnected Orders connects Etsy once", async ({ page }) => {
  ...
  await page.route("**/api/etsy-connection", r => r.request().method() === "POST" ? (begins++, r.fulfill({ status: 503, json: { error: "Navigation intercepted" } })) : r.fulfill({ json: { status: "disconnected" } }));
  await open(page); const button = page.locator(".etsy-import-button"); await expect(button).toHaveText("Connect Etsy Shop"); await button.click(); await button.click({ force: true }); await expect.poll(() => begins).toBe(1);
});
...
test("connected import reports stages and completion while preserving selection", async ({ page }) => {
  ...
  const button = page.locator(".etsy-import-button"); await expect(button).toHaveText("Import from Etsy"); await button.click(); await button.click({ force: true }); await expect.poll(() => page.evaluate(() => calls)).toBe(1);
  ...
  const progress = page.locator(".etsy-import-progress"); await expect(progress).toHaveAttribute("value", "6"); await expect(progress).toHaveAttribute("max", "14");
...
test("customization keeps actions and OAuth consumes only Etsy param", async ({ page }) => {
  ...
  const card = page.locator(".database-order-item-card"); await expect(card.getByText("Customization needed", { exact: false })).toBeVisible(); await expect(card.getByRole("button", { name: "Item actions" })).toBeVisible();
});
