...
describe("workspace font alias store", () => {
  it("returns only browser-safe alias and target-font metadata", async () => {
    ...
    const supabase = createAliasSupabase({
      aliases: [{
        ...
        font_id: "font-1", revision: 7, created_by: "operator-1", updated_by: "operator-2", etsy_import_diagnostics: { token: "private" },
    ...
    await expect(listWorkspaceFontAliases({ workspaceId: "workspace-1", supabase })).resolves.toEqual([{
      id: "alias-1", aliasName: " Lemonade ", normalizedAlias: "lemonade", fontId: "font-1", revision: 7,
  ...
  it("sends the server-canonical alias and authenticated operator to the mapping RPC", async () => {
    ...
    const supabase = createAliasSupabase({
      rpcResult: { data: {
        ...
        alias_revision: 8, font_display_name: "Super Boys", font_archived_at: null, font_deleted_at: null,
      }, error: null },
    });
    ...
    const result = await mapWorkspaceFontAlias({
      ...
      normalizedAlias: "attacker-controlled", fontId: "font-1", expectedAliasRevision: 7, supabase,
    });
    ...
    expect(supabase.rpcCall).toEqual({
      ...
      args: expect.objectContaining({
        ...
        p_normalized_alias: "super boy", p_font_id: "font-1", p_expected_alias_revision: 7,
      }),
    });
    expect(result.alias).toMatchObject({
      id: "alias-1", aliasName: "  S\u{FF35}\u{FF50}\u{FF45}\u{FF52}   Boy ", normalizedAlias: "super boy", fontId: "font-1", revision: 8,
  ...
  it("surfaces authoritative design invalidation state from the mapping RPC", async () => {
    const supabase = createAliasSupabase({
      rpcResult: { data: {
        alias_id: "alias-1",
        alias_name: "Lemonade",
        normalized_alias: "lemonade",
        alias_revision: 2,
        font_id: "font-1",
