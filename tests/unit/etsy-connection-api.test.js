...
describe("Etsy connection API", () => {
  ...
  it("marks the authorization cookie Secure in production", async () => { const previous = process.env.NODE_ENV; process.env.NODE_ENV = "production"; mocks.begin.mockReturnValue({ authorizeUrl: "https://etsy.test", sealedContext: "sealed" }); const res = response(); await handler({ method: "POST", body: { action: "beginAuthorization" } }, res); expect(res.headers["Set-Cookie"]).toContain("; Secure;"); process.env.NODE_ENV = previous; });
